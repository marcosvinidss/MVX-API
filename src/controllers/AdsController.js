...
module.exports = {
    ...
    for (let i in adsData) {
        ...
        if (adsData[i].images && adsData[i].images.length > 0) {
            // tenta encontrar a imagem default
            let defaultImg = adsData[i].images.find(e => e.default);
            if (defaultImg) {
                image = `${process.env.BASE}/media/${defaultImg.url}`;
            } else {
                // pega a primeira imagem se não existir default
                image = `${process.env.BASE}/media/${adsData[i].images[0].url}`;
            }
        } else {
            image = null; // ou pode colocar a default, se quiser
        }
  ...
  getItem: async (req, res) => {
    ...
    if (ad.images && ad.images.length > 0) {
        ...
        images = []; // ou coloca default aqui, se quiser
    }
...
,
    ...
    editAction: async (req, res) => {
        ...
        if (priceneg) updates.priceNegotiable = priceneg === 'true';
